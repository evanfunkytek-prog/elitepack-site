...
/* ===== WhatsApp floating chat widget (EliteBox Packaging) ===== */
(function(){
  'use strict';
  ...
  waReady(function(){
    ...
    function pageMsg(){
      ...
      if(!content){ return "Hello EliteBox Packaging! I'm interested in your custom packaging boxes and would like a quote. Can we chat?"; }
      return 'Hello EliteBox Packaging, I was just viewing "'+(cleanTitle()||'your page')+'" and would like to ask about it. Can we chat?';
    }
    function waLink(){ return 'https://wa.me/'+PHONE+'?text='+encodeURIComponent(pageMsg()); }
    var css='#eb-wa{position:fixed;right:18px;bottom:18px;z-index:9990;font-family:inherit;font-size:14px}#eb-wa .eb-wa-btn{position:relative;display:flex;align-items:center;justify-content:center;width:58px;height:58px;border-radius:50%;background:#25D366;color:#fff;box-shadow:0 6px 18px rgba(0,0,0,.18);text-decoration:none;transition:transform .2s ease}#eb-wa .eb-wa-btn:hover{transform:scale(1.06)}#eb-wa .eb-wa-pulse{position:absolute;inset:0;border-radius:50%;background:#25D366;opacity:.45;animation:ebwa 2.2s ease-out infinite;pointer-events:none}@keyframes ebwa{0%{transform:scale(1);opacity:.5}70%{transform:scale(1.55);opacity:0}100%{transform:scale(1.55);opacity:0}}@media(prefers-reduced-motion:reduce){#eb-wa .eb-wa-pulse{animation:none}}#eb-wa .eb-wa-bubble{position:absolute;right:72px;bottom:0;width:252px;background:#fff;border:1px solid #e6e6e6;border-radius:14px;padding:12px 14px 10px;box-shadow:0 8px 24px rgba(0,0,0,.14);display:none;text-align:left}#eb-wa .eb-wa-bubble.open{display:block}#eb-wa .eb-wa-bubble .eb-wa-x{position:absolute;top:6px;right:9px;border:none;background:none;color:#999;font-size:14px;line-height:1;cursor:pointer;padding:2px}#eb-wa .eb-wa-bubble p{margin:2px 12px 10px 0;font-size:13px;line-height:1.5;color:#333}#eb-wa .eb-wa-act{display:flex;gap:8px}#eb-wa .eb-wa-act a{flex:1;text-align:center;border-radius:999px;padding:8px 6px;font-size:13px;font-weight:600;text-decoration:none}#eb-wa a.eb-wa-go{background:#25D366;color:#fff}#eb-wa .eb-wa-app{font-size:11.5px;color:#777;margin-top:9px;text-align:center}#eb-wa .eb-wa-app a{color:#128C7E;text-decoration:underline;padding:0}';
    ...
    wrap.querySelector('.eb-wa-x').addEventListener('click', function(){ closeBubble(); try{ sessionStorage.setItem(seenKey,'1'); }catch(e){} });
    wrap.addEventListener('mouseenter', openBubble);
    wrap.addEventListener('mouseleave', closeBubble);
    document.body.appendChild(wrap);
    var seen=false; try{ seen=sessionStorage.getItem(seenKey)==='1'; }catch(e){}
    if(!seen){ setTimeout(openBubble, 2400); }
  });
})();
